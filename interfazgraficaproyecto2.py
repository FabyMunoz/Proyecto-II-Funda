...
def ascii_increment5_info(ch):
    ch = normalize_text(ch[:1])
    if not ch:
        return None
    ascii_value = ord(ch)
    low_nibble = ascii_value & 0x0F
    incremented = (low_nibble + 5) & 0x0F
    return {
        "char": ch,
        "ascii": ascii_value,
        "input_bits": format(low_nibble, "04b"),
        "expected_bits": format(incremented, "04b"),
        "expected_value": incremented,
    }
    ...
    def parse_line(self, line):
        ...
        if p[0] == "INC_UPDATE":
            return {
                ...
                "expected_bits": p[5] if len(p) > 5 else "",
                "hardware_bits": p[6] if len(p) > 6 else "",
                "raw_outputs": p[7] if len(p) > 7 else "",
            }
        if p[0] == "SWITCH":
...
class StrangerTecApp(tk.Tk):
    ...
    def _build_ui(self):
        tk.Label(self, text="StrangerTEC Morse Translator", bg="#0f172a", fg="#e5e7eb", font=("Arial", 22, "bold")).pack(pady=10)
        container = tk.Frame(self, bg="#0f172a")
        container.pack(fill="both", expand=True, padx=12, pady=8)
        left = tk.Frame(container, bg="#111827", bd=1, relief="solid")
        center = tk.Frame(container, bg="#111827", bd=1, relief="solid")
        right = tk.Frame(container, bg="#111827", bd=1, relief="solid")
        left.pack(side="left", fill="both", expand=False, padx=6)
    ...
    def _build_game_panel(self, parent):
        ...
        ttk.Separator(parent).pack(fill="x", padx=10, pady=8)
        tk.Label(parent, text="Incrementador físico +5", bg="#111827", fg="#93c5fd", font=("Arial", 11, "bold")).pack(anchor="w", padx=10)
        tk.Label(parent, textvariable=self.incrementer_var, bg="#1f2937", fg="#bfdbfe", font=("Consolas", 11), wraplength=430, justify="left").pack(fill="x", padx=10, pady=5)
        ...
        tk.Button(row4, text="Probar valor manual", command=self.test_incrementer_value, bg="#38bdf8").pack(side="left", expand=True, fill="x", padx=3)
        tk.Button(row4, text="Limpiar incrementador", command=self.clear_incrementer, bg="#64748b", fg="white").pack(side="left", expand=True, fill="x", padx=3)
        self.results_box = tk.Text(parent, height=10, bg="#020617", fg="#e5e7eb", font=("Consolas", 10))
    ...
    def _build_alphabet_panel(self, parent):
        ...
        chars = list("ABCDEFGHIJKLMNOPQRSTUVWXYZ") + list("0123456789") + ["+", "-", "ESP"]
        for idx, ch in enumerate(chars):
            cmd_char = " " if ch == "ESP" else ch
            tk.Button(grid, text=ch, width=7, command=lambda x=cmd_char: self.append_pc_char(x), bg="#334155", fg="white").grid(row=idx//6, column=idx%6, padx=3, pady=3)
        ttk.Separator(parent).pack(fill="x", padx=10, pady=8)
    ...
    def handle_pico_msg(self, msg):
        ...
        if mtype == "HELLO":
            ...
            self.incrementer_var.set(f"Incrementador +5: switch {state}.\nEntradas: A=GP0 B=GP1 C=GP2 D=GP3 | Salidas: S3=GP4 S2=GP5 S1=GP6 S0=D'")
    ...
    def clear_incrementer(self):
        ...
        self.incrementer_var.set("Incrementador +5: salidas limpiadas desde la interfaz.")
    def update_incrementer_from_pico(self, msg):
        enabled = msg.get("enabled") == "1"
        char = msg.get("char", "")
        ascii_text = msg.get("ascii", "")
        input_bits = msg.get("input_bits", "")
        expected_bits = msg.get("expected_bits", "")
        hardware_bits = msg.get("hardware_bits", "")
        raw_outputs = msg.get("raw_outputs", "")
        ok = "OK" if hardware_bits == expected_bits else "REVISAR"
        label = "activo" if enabled else "apagado por switch"
        self.incrementer_var.set(
            f"Incrementador +5 ({label})\n"
            f"Letra/valor: {char or 'manual'} | ASCII: {ascii_text}\n"
            f"Entrada ABCD: {input_bits} -> esperado S3S2S1S0: {expected_bits}\n"
            f"Circuito S3S2S1S0: {hardware_bits} ({ok}) | Pines leidos: {raw_outputs}"
        )
        self.incrementer_last = msg
        self.write_line(f"Incrementador +5: entrada {input_bits}, esperado {expected_bits}, circuito {hardware_bits} [{ok}]")
    ...
    def write_result(self, label, result):
        ...
        for ch in result["received_text"]:
            info = ascii_increment5_info(ch)
            if info:
                inc_lines.append(f"  {info['char']}: ASCII {info['ascii']} | ABCD {info['input_bits']} | +5 {info['expected_bits']} ({info['expected_value']})")
