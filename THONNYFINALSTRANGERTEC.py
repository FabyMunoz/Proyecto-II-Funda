...
# Incrementador +5 del Proyecto II.
# Entradas al circuito: A=GP0, B=GP1, C=GP2, D=GP3.
# Las salidas S0-S3 salen de los chips logicos directo a LEDs fisicos.
# La Pico NO lee S0-S3 de vuelta; solo calcula el resultado esperado
# para mostrarlo en la pantalla de la PC.
PIN_INC_A = 0
PIN_INC_B = 1
PIN_INC_C = 2
PIN_INC_D = 3
...
inc_inputs = [
    Pin(PIN_INC_A, Pin.OUT),
    Pin(PIN_INC_B, Pin.OUT),
    Pin(PIN_INC_C, Pin.OUT),
    Pin(PIN_INC_D, Pin.OUT),
]
...
# INCREMENTADOR +5
def bits4(value):
    return "{:04b}".format(value & 0x0F)

def incrementer_enabled():
    return force_incrementer or switch_mode.value() == 1

def clear_incrementer():
    global last_increment_value
    last_increment_value = 0
    for pin in inc_inputs:
        pin.value(0)

def drive_incrementer_value(value, source_char=""):
    global last_increment_value

    value = value & 0x0F
    last_increment_value = value

    if not incrementer_enabled():
        clear_incrementer()
        send_incrementer_update(False, source_char, value)
        return

    # A es el bit mas significativo de los 4 bits bajos; D es el menos significativo.
    inc_inputs[0].value((value >> 3) & 1)
    inc_inputs[1].value((value >> 2) & 1)
    inc_inputs[2].value((value >> 1) & 1)
    inc_inputs[3].value(value & 1)
    time.sleep_ms(3)

    send_incrementer_update(True, source_char, value)

def drive_incrementer_from_char(ch):
    if not ch or ch == "?":
        return
    ascii_value = ord(ch)
    drive_incrementer_value(ascii_value & 0x0F, ch)

def send_incrementer_update(enabled, source_char, input_value):
    ascii_text = ord(source_char) if source_char else "manual"
    expected = (input_value + 5) & 0x0F
    send_line("INC_UPDATE|{}|{}|{}|{}|{}|{}".format(
        1 if enabled else 0,
        source_char,
        ascii_text,
        bits4(input_value),
...
def handle_line(line):
    ...
    if mtype == "PING":
        ...
        try:
            ...
            value = 0
        drive_incrementer_value(value, "")
        send_line("ACK|TEST_INC|{}".format(value & 0x0F))

    elif mtype == "CLEAR_INC":
        clear_incrementer()
        send_incrementer_update(incrementer_enabled(), "", 0)
        send_line("ACK|CLEAR_INC")

    elif mtype == "FORCE_INC":
        set_force_incrementer(parts[1] == "1" if len(parts) > 1 else False)
...
def decode_current_symbol():
    ...
    drive_incrementer_from_char(ch)
    send_input_update()
...
# SWITCH
def set_force_incrementer(enabled):
    global force_incrementer
    force_incrementer = enabled
    send_line("SWITCH|{}".format("on" if incrementer_enabled() else "off"))
    if incrementer_enabled():
        drive_incrementer_value(last_increment_value, "")
        send_line("ACK|FORCE_INC|1")
    else:
        clear_incrementer()
        send_incrementer_update(False, "", 0)
        send_line("ACK|FORCE_INC|0")
