...
# Retorno desde el circuito: S3=GP4, S2=GP5, S1=GP6. S0 es D' segun el cableado.
PIN_INC_A = 0
PIN_INC_B = 1
PIN_INC_C = 2
PIN_INC_D = 3
...
inc_s3 = Pin(PIN_INC_S3, Pin.IN, Pin.PULL_DOWN)
inc_s2 = Pin(PIN_INC_S2, Pin.IN, Pin.PULL_DOWN)
inc_s1 = Pin(PIN_INC_S1, Pin.IN, Pin.PULL_DOWN)
...
def drive_incrementer_value(value, source_char=""):
    ...
    if not incrementer_enabled():
        clear_incrementer()
        send_incrementer_update(False, source_char, value, 0, "000")
        return
    ...
    expected = (value + 5) & 0x0F
    s3 = inc_s3.value()
    s2 = inc_s2.value()
    s1 = inc_s1.value()
    s0 = 0 if (value & 1) else 1
    hardware = (s3 << 3) | (s2 << 2) | (s1 << 1) | s0
    send_incrementer_update(True, source_char, value, hardware, "{}{}{}".format(s3, s2, s1))
...
def send_incrementer_update(enabled, source_char, input_value, hardware_value, raw_outputs):
...
def handle_line(line):
    ...
    if mtype == "PING":
        ...
        clear_incrementer()
        send_incrementer_update(incrementer_enabled(), "", 0, 0, "000")
...
def check_switch():
    ...
    if value != last_switch_state:
        ...
        if value:
            ...
            clear_incrementer()
            send_incrementer_update(False, "", 0, 0, "000")
